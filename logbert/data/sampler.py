...
class BucketBatchSampler(Sampler):
    ...
    """
    def __init__(self, data_source, batch_size, drop_last=False, seed=None, mode="train", resume_from_epoch=0):
        try:
        ...
        self.lengths = np.array(data_source.lengths, dtype=np.int64)
    ...
    def __iter__(self):
        self.epoch += 1
        logger.debug(f"BucketBatchSampler epoch {self.epoch}")
        ...
        logger.debug(f"[SAMPLER] WORLD SIZE: {world_size}, RANK: {rank}, TOTAL BATCHES: {len(batches)}")
        ...
        if world_size > 1:
            ...
            if padded_size > total_batches:
                logger.debug(f"[SAMPLER] Padding from {total_batches} to {padded_size} batches for rank {rank}")
            ...
            logger.debug(f"[SAMPLER] RANK {rank}: {len(batches)} batches after distribution")
