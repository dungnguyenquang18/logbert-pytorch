...
def make_batch(B=4, L=20, vocab=30):
    ...
    return {
        ...
        "mlm_labels": torch.randint(5, vocab, (B, L)),
    }
...
def test_forward_returns_all_components():
    ...
    for k in ("loss", "loss_cls", "loss_mlm", "loss_l1"):
        assert out[k] is not None and out[k].dim() == 0
    # composition: loss = cls + lambda_l1*l1 + alpha_mlm*mlm
    expected = out["loss_cls"] + 1e-4 * out["loss_l1"] + 0.1 * out["loss_mlm"]
    assert torch.allclose(out["loss"], expected, atol=1e-6)


def test_heads_disabled():
    m = LogBertClassifier(small_cfg(use_mlm=False, use_l1=False))
    out = m(**make_batch())
    assert out["loss_mlm"] is None and out["loss_l1"] is None
    assert torch.allclose(out["loss"], out["loss_cls"])


def test_no_labels_predict_mode():
    m = LogBertClassifier(small_cfg(use_l1=False))
    b = make_batch(); b["labels"] = None; b["mlm_labels"] = None
    out = m(**b)
